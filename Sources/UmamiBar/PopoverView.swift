...
struct PopoverView: View {
    ...
    private var statGrid: some View {
        ...
        return LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible())], spacing: 10) {
            ...
            StatCard(title: "Bounce rate", value: percent(bounceRate), delta: delta(bounceRate, prevBounceRate))
    ...
    private var chart: some View {
        Chart {
            if let series = store.series {
                ...
                ForEach(Array(series.sessions.enumerated()), id: \.offset) { _, point in
                    if let date = point.date {
                        LineMark(x: .value("Time", date), y: .value("Sessions", point.y))
    ...
    private var footer: some View {
        HStack(spacing: 6) {
            ...
            .disabled(store.isLoading || !store.settings.isConfigured)

            Menu {
                Button("Open Umami dashboard") {
                    if let url = URL(string: store.settings.config.dashboardURL) {
